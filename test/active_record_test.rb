...
class Person < ActiveRecord::Base
  yattr_encrypted :email, :key => 'a secret key'
  yattr_encrypted :credentials, :key => Proc.new { |user| Encryptor.encrypt(:value => user.salt, :key => 'some private key') }, :marshal => true
...
class ActiveRecordTest < Test::Unit::TestCase
  ...
  def test_should_encode_by_default
    assert Person.yattr_encrypted_options[:encode]
  end
